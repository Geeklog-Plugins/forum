...
$sort        = (int) Input::fRequest('sort', 0);
$order       = (int) Input::fRequest('order', 0);
$query       = isset($_REQUEST['query']) ? addslashes(strip_tags(Input::request('query'))) : '';
$forum       = (int) Input::fRequest('forum', 0);
$cat_id      = (int) Input::fRequest('cat_id', 0);
$category    = (int) Input::fRequest('category', 0); // Used for display category forum index
$populartype = Input::fRequest('populartype', '');
...
if ($op == 'newposts' AND !COM_isAnonUser()) {
    ...
    $blocks = ['sort_desc', 'sort_desc_on', 'sort_asc', 'sort_asc_on'];
...
if ($forum > 0) {
    ...
    $topicCount = DB_count($_TABLES['forum_topic'], ['pid', 'forum'], [0, $forum]);
} else {
...
if ($forum == 0) {
    ...
    $blocks = ['newpost_link', 'markread_link', 'categorymenu_link'];
    foreach ($blocks as $block) {
...
if ($forum > 0) {
    ...
    $blocks = [
        'new_icon', 'quiet_icon', 'active_icon', 'normal_icon', 'normalnew_icon', 'sticky_icon',
        'stickynew_icon', 'locked_icon', 'lockednew_icon', 'sort_desc', 'sort_desc_on',
        'sort_asc', 'sort_asc_on'
    ];
    foreach ($blocks as $block) {
        $topiclisting->set_block('forum_icons', $block);
    }
    
    $blocks = ['newtopic_link', 'subscribeforum_link', 'forummenu_link'];
    foreach ($blocks as $block) {
...
if (isset($_REQUEST['forum'])) {
    $forum_id = (int) Input::fRequest('forum', 0);
